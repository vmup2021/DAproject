...
Station* Graph::findStationByName(string stationName = "") {
    for(auto it = allStations.begin(); it != allStations.end(); it++){
        if (it->getName() == stationName) { return it.base(); }
    }
...
void Graph::readStations(string lineName) {
    ...
    while(getline(file,line)) {
        ...
        while ((pos = line.find(delimiter)) != string::npos) {
            ...
            if (line.find(delimiter) == string::npos) array_of_fields.push_back(line);
        }
        ...
        if(lineName == "all" || lineName == array_of_fields[4]){
            Station *station_ = new Station(array_of_fields[0], array_of_fields[1], array_of_fields[2], array_of_fields[3], array_of_fields[4]);
...
vector<Edge *> Graph::bfsFindPath(string source, string destination) {
...
//            cout << current_station->getName() << " to " << next_station->getName() << " c="<< e->getCapacity() << " f=" << e->getFlow() << endl;
...
vector<Edge*> Graph::returningPath(Station *sink) const {
    vector<Edge*> path;
...
//T2.1
int Graph::maxFlow(string source, string destination) {
...
void Graph::topK(int n) {
    ...
    for(Station &s1 : allStations){
        for(Station &s2 : allStations) {
            ...
            if(bfsFindPath(s1.getName(), s2.getName()).empty()) continue;
...
//T2.4
int Graph::entireMaxFlow(string sink) {
    ...
    Station *superSource = new Station("superSource", "d", "m", "t", "l");
...
//T3.1
int Graph::shortestPathCost(string source, string destination) {
    setAllStationsUnvisited();
    ...
    auto comp = [this](Station* a, Station* b){ return a->getDist() < b->getDist(); };

    //Priority queue to pick always the station with lowest distance
    ...
    while(!pq.empty()){
        ...
        if(current_station == d) return d->getDist();

        for(auto &e : current_station->adj){
            Station *next_station = e->getDestination();

            if(next_station->getVisited()) continue;

            int new_dist = current_station->getDist() + e->getCapacity() * (e->getType() == "STANDARD" ? 2 : 4);
//            cout << next_station->getName() << " old:" << next_station->getDist() << " possibly new:" << new_dist << " e:" << e->getCapacity() << endl;
